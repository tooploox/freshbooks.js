  ...
  this._setXML("contractor.list", options, function(xml) {
    self.freshbooks._get(xml, function(err, xml) {
      if (err) {
        ...
        var contractor = xml.get("//xmlns:contractors", self.freshbooks.ns);
        var options = {
          page: contractor.attr("page").value(),
          per_page: contractor.attr("per_page").value(),
          pages: contractor.attr("pages").value(),
          total: contractor.attr("total").value()
        };
        var contractors = [];
