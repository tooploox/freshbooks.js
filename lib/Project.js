  ...
  this._setXML("project.list", options, function(xml) {
    self.freshbooks._get(xml, function(err, xml) {
      if (null !== err) {
        ...
        var project = xml.get("//xmlns:projects", self.freshbooks.ns);
        var options = {
          page: project.attr("page").value(),
          per_page: project.attr("per_page").value(),
          pages: project.attr("pages").value(),
          total: project.attr("total").value()
        };
        var projects = [];
